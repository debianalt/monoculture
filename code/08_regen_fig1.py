...
COLOR_MAP = {"SAS": "#D55E00", "Asoc": "#56B4E9", "Coop": "#009E73",
             "SRL": "#E69F00", "Fund": "#CC79A7", "Otra": "#999999"}
HATCH_MAP = {"SAS": "xxx", "Asoc": "///", "Coop": "...", "SRL": "", "Fund": "\\\\\\", "Otra": ""}
NAME_MAP = {"SAS": "SAS", "Asoc": "Associations", "Coop": "Cooperatives",
            "SRL": "Commercial", "Fund": "Services", "Otra": "Other"}
...
for cl in sorted(clusters.unique()):
    ...
    ax.scatter(sample.iloc[:, 0], sample.iloc[:, 1], c=CLUSTER_COLORS[cl], s=3, alpha=0.05, rasterized=True)
    # KDE density contours per cluster
    xc = row_coords.loc[mask].iloc[:, 0].values
    yc = row_coords.loc[mask].iloc[:, 1].values
    if len(xc) > 50:
        try:
            kde = gaussian_kde(np.vstack([xc, yc]))
            xg = np.linspace(xc.min() - 0.3, xc.max() + 0.3, 80)
            yg = np.linspace(yc.min() - 0.3, yc.max() + 0.3, 80)
            Xg, Yg = np.meshgrid(xg, yg)
            Zg = kde(np.vstack([Xg.ravel(), Yg.ravel()])).reshape(Xg.shape)
            ax.contour(Xg, Yg, Zg, levels=3, colors=[CLUSTER_COLORS[cl]], linewidths=0.6, alpha=0.5)
        except Exception:
            pass
...
legend_order = ["Commercial", "Associations", "Cooperatives", "Services", "SAS"]
cl_by_name = {CLUSTER_NAMES[c]: c for c in CLUSTER_COLORS}
handles = [mpatches.Patch(color=CLUSTER_COLORS[cl_by_name[n]], hatch=CLUSTER_HATCHES.get(cl_by_name[n], ""),
                          edgecolor="#555555", label=n) for n in legend_order if n in cl_by_name]
ax.legend(handles=handles, loc="upper center", bbox_to_anchor=(0.5, -0.08),
          ncol=len(handles), fontsize=7, framealpha=0.95, edgecolor="#ccc")
plt.subplots_adjust(bottom=0.13)
plt.savefig(PROJECT / "figures" / "Fig2.png", dpi=DPI)
plt.savefig(PROJECT / "figures" / "Fig2.tiff", dpi=DPI, format="tiff", pil_kwargs={"compression": "tiff_lzw"})
plt.close()
print("Fig2 OK (MCA biplot)", flush=True)
